    ...
    if show_markup:
        ...
        if func_type in ['Quadratic', 'Hard Exponential']:
            ...
            for i in range(3):
                ...
                # Shifted orange arcs further right (from 0.65 to 0.72)
                ax.annotate("", xy=(0.72, ye), xytext=(0.72, ys),
                ...
                # Shifted orange text further right (from 0.85 to 0.88)
                ax.text(0.88, ymid, lbl, color='#FF4500', fontsize=10, va='center', fontweight='bold')
...
def create_pdf_bytes():
    ...
    with PdfPages(buffer) as pdf:
        ...
        for i, prob in enumerate(problems):
            # Reduced to 3 slots per column (6 questions per page) to fix vertical overlapping
            if current_slot >= 3:
        ...
        for page_idx, page_items in enumerate(app_pages):
            ...
            for item in page_items:
                ...
                # Increased vertical gap multiplier from 0.22 to 0.28 to match new 6-item layout
                y_pos = 0.88 - (slot * 0.28) 
                
                steps_clean = prob_data[3].replace("**", "")
                
                # Wrapped at 45 characters to fix horizontal column bleed
                wrapped_lines = [textwrap.fill(line, width=45) for line in steps_clean.split('\n')]
                final_text = '\n'.join(wrapped_lines)
                
                ax_app.text(x_pos, y_pos, f"Q{q_num+1}:\n{final_text}", fontsize=8.5, va='top')
