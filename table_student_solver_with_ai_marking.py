    ...
    with PdfPages(buffer) as pdf:
        ...
        for page_idx, page_items in enumerate(app_pages):
            ...
            for item in page_items:
                ...
                # Wrap each line to 55 characters to perfectly fit the two-column layout
                wrapped_lines = [textwrap.fill(line, width=55) for line in steps_clean.split('\n')]
                final_text = '\n'.join(wrapped_lines)
                
                ax_app.text(x_pos, y_pos, f"Q{q_num+1}:\n{final_text}", fontsize=9, va='top')
