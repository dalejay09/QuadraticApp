...
if st.session_state.generating:
    ...
    if st.session_state.answered:
        ...
        if st.button("Next Parabola", use_container_width=True):
            st.session_state.generating = True
            st.rerun()                 f"3. ${fmt_num(py - k)} = a({fmt_num((px - h)**2)}) \\Rightarrow a = {fmt_num(a)}$\n\n**${eq}$**")
        correct = ['Vertex']
...
if st.session_state.generating:
    ...
    if st.session_state.feedback:
        ...
        px1, py1 = fmt_num(points[1][0]), fmt_num(points[1][1])
        px2, py2 = fmt_num(points[2][0]), fmt_num(points[2][1])
        steps = (f"**Standard Form**\n\n1. Y-Int $(0, c) = (0, {c}) \\Rightarrow c = {c}$.\n"
                 f"2. Sub $({px1}, {py1})$: ${py1} = {fmt_num(px1**2)}a + {px1}b + {c}$\n"
                 f"3. Sub $({px2}, {py2})$: ${py2} = {fmt_num(px2**2)}a - b + {c}$\n"
                 f"4. Solving system yields: $a = {fmt_num(a)}, b = {b}$\n\n**${eq}$**")
        correct = ['Standard']
